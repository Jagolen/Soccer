...
with st.spinner("Loading"):
	...
	if selected_sub_page == "Train a model":
		...
		selected_att = st.multiselect('Attr==True', bool_columns)
		for sa in selected_att:
			df_train = df_train[df_train[sa] == True]

		# Select which attributes should be False
		selected_att_f = st.multiselect('Attr==False', bool_columns, [])
		...
		selected_att_gt_zero = st.multiselect('Attr>0', float_columns, [])
		for sa in selected_att_gt_zero:
			df_train = df_train[df[sa] > 0]
