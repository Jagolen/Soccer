...
with st.spinner("Loading"):
	...
	if selected_sub_page == "Test a model":
		...
		if how_many_models_test == "Test a model":
			...
			stat_df_log_coef.sort_values(by="Coefficient value", inplace=True, ascending=False)
			...
			stat_df_lin_coef.sort_values(by="Coefficient value", inplace=True, ascending=False)
		...
		if how_many_models_test == "Compare two models":
			...
			with log_stats2:
				...
				stat_df_log_coef2.sort_values(by="Coefficient value", inplace=True, ascending=False)
	...
	if selected_sub_page == "Pass Assesment":
		...
		coeff_val_df_log = pd.DataFrame()
		coeff_val_df_log["Var"] = model_pass_log.params
		coeff_val_df_log["PosNeg"] = ['Positive coefficient' if x>=0 else 'Negative Coefficient' for x in coeff_val_df_log['Var']]
		...
		if type_of_model == "loglin":
			...
			coeff_val_df_lin = pd.DataFrame()
			coeff_val_df_lin["Var"] = model_pass_lin.params
			coeff_val_df_lin["PosNeg"] = ['Positive coefficient' if x>=0 else 'Negative Coefficient' for x in coeff_val_df_lin['Var']]
		for i in range(10):
			...
			if type_of_model == "loglin":
				...
				with coef_cont_log:
					...
					temp_log = coeff_val_df_log.copy()
					temp_log = temp_log[temp_log.index.isin(stat_log.index)]
					temp_log = temp_log["PosNeg"]
					st.table(stat_log)
					st.table(temp_log)
				with coef_cont_lin:
					st.write("**Top 3 Coefficient Contributions (Lin Model)**")
					stat_lin = stat_lin.astype(float).nlargest(3)
					temp_lin = coeff_val_df_lin.copy()
					temp_lin = temp_lin[temp_lin.index.isin(stat_lin.index)]
					temp_lin = temp_lin["PosNeg"]
					st.table(stat_lin)
					st.table(temp_lin)
