...
with st.spinner("Loading"):
	...
	if selected_sub_page == 'Visualize Model':
		...
		if type_of_model == "loglin" or type_of_model2 == "loglin":
			model_names_lin = []
		...
		if test_old_model == "xT Positional":
			...
			if nr_models == "Compare":
				df2[['time_from_chain_start', 'time_difference']] = 0, 0
		...
		if st.session_state.train_the_model:
			if target_attribute:
				if model_type == "Linear Regression Model":
					...
					lin_model_scaled.save(output_name_scaled, remove_data=True)
				...
				if model_type == "Logistic Regression Model":
					...
					log_model_scaled.save(output_name_scaled, remove_data=True)
					st.write(f"Model has been saved at {output_name} and {output_name_scaled}")
					st.session_state.train_the_model = False
			else:
