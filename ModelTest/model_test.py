...
for i in range(11):
    ...
    path = os.path.join(str(pathlib.Path().resolve().parents[0]), 'modeltest', file_name)
    with open(path) as f:
        data = json.load(f)
...
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size = 0.9, random_state = 777, stratify = y)
themodel = LogisticRegressionCV(solver='liblinear', max_iter=200)
themodel.fit(X_train, y_train)
y_pred_proba = themodel.predict_proba(X)[::,1]
...
chain = df.loc[df["possesion_chain"] == 2]
#get passes
passes_in = passes.loc[df["possesion_chain"] == 2]
